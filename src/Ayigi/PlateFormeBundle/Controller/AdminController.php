...
//----------------------- Manage Client --------------------
    ...
    public function updateClientAction($id, Request $request)
    {
        ...
        $clientUser = $em->getRepository('AyigiClientBundle:Client')->find($id);

        $form = $this->get('form.factory')->create(ClientType::class, $clientUser);
        ...
        if ($request->getMethod()=='POST') {
            ...
            if($form->isValid())
            {
                $clientUser= $form->getData();

                $em->flush();

                return $this->redirect($this->generateUrl('ayigi_plate_forme_admin_liste_client'));
            }
        }
